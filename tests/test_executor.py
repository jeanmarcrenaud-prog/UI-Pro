...
NOTE: Most tests are SKIPPED because the executor.run() function has been
replaced by the multi-language CodeExecutor class. These tests tested a
subprocess wrapper that no longer exists. Rewrite tests against
CodeExecutor.run() when needed.
"""
...
from backend.domain.core.executor import run


class TestExecutorBasic:
    """Test basic executor functionality"""

    @pytest.mark.skip(reason="run() API replaced by CodeExecutor")
    ...
    def test_run_with_error_output(self, mock_run):
        ...
        mock_run.return_value = Mock(
            returncode=1, stdout="", stderr="Error message here"
        )
...
class TestTimeoutHandling:
    ...
    def test_run_with_nonzero_exit(self, mock_run):
        ...
        mock_run.return_value = Mock(
            returncode=1, stdout="", stderr="Process failed with exit code 1"
        )
...
class TestSubprocessConfiguration:
    ...
    def test_execution_timeouted(self, mock_run):
        ...
        mock_run.side_effect = subprocess.TimeoutExpired(
            cmd=["python", "workspace/app.py"], timeout=60
        )
    ...
    def test_subprocess_failure(self, mock_run):
        ...
        mock_run.side_effect = FileNotFoundError("python", None, "Python not found")
...
class TestWorkspaceManagement:
    ...
    @pytest.mark.skip(reason="run() no longer creates workspace via subprocess")
    @patch("subprocess.run")
...
class TestErrorHandling:
    ...
    def test_binary_output(self, mock_run):
        ...
        mock_run.return_value = Mock(returncode=0, stdout=b"binary content", stderr="")
...
class TestLogging:
    ...
    @pytest.mark.skip(reason="executor module no longer has subprocess logger")
    @patch("subprocess.run")
