...
class TaskPlanner:
    ...
    async def generate_plan(self, intent_description: str, current_state: EditorState) -> List[Action]:
        ...
        context_str = (
            f"Active Files: {current_state.active_file if current_state.active_file else 'None'}\n"
            f"Cursor Position: "
            f"{f'Line {current_state.cursor.line}, Column {current_state.cursor.column}' if current_state.cursor else 'None'}\n"
