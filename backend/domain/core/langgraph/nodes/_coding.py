...
async def coding_node(state: AgentState) -> dict[str, Any]:
    ...
    prompt_parts.append(
        ...
        "- CRITICAL: Generate at most 2 files. Split code only when truly necessary (e.g. a library + a main script). Prefer a single file.\n\n"
        ...
        "CHARACTER QUALITY — avoid invisible or special Unicode characters that break parsing:\n"
        "- NEVER use fancy Unicode symbols: ♦, ●, →, ⇒, ★, bullet points (•), or any non-ASCII arrows or geometric shapes\n"
        "- NEVER use typographic quotes (\u201c, \u201d, \u2018, \u2019) — use ASCII straight quotes (\", ') only\n"
        "- NEVER emit control characters (\\x00-\\x1F) — they are invisible and cause SyntaxError\n"
        "- For degree Celsius or similar: write \"°C\" as \" deg C\" or \" celsius\" in plain text strings — never use special symbols (♦, ●) as stand-ins\n"
        "- Use ASCII-only characters in all source code and comments. Stick to printable, plain-text symbols\n\n"
