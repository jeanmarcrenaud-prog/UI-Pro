...
def should_continue(state: AgentState) -> Literal["fix_code", "end"]:
    ...
    # Priority 0: reviewing_node detected empty code (structured flag
    # set by the no-code short-circuit; avoids string-matching the issue
    # text, which was a fragile duplicate of the detection logic).
    if review is not None and review.get("no_code"):
        _emit_step(
