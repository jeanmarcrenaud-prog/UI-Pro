...
class DiscoveredModel:
    ...
    size_vram_gb: float | None = None  # VRAM used if loaded
...
class ModelDiscovery:
    ...
    def _infer_strengths(name: str) -> list[TaskType]:
        ...
        if any(
            x in name_lower for x in ["llama", "mistral", "qwen", "deepseek", "opus"]
        ):
    ...
    def _get_loaded_models_ollama(self) -> dict[str, dict]:
        ...
        try:
            endpoint = self._get_endpoints()["ollama"]["url"].replace(
                "/api/tags", "/api/ps"
            )
            ...
            logger.debug(
                f"[ModelDiscovery] Ollama has {len(loaded_map)} models loaded in memory"
            )
            return loaded_map
        except Exception as e:
                ...
                try:
                    ...
                    for model in models:
                        if model.backend == "ollama" and model.name in loaded_ollama:
                            ...
                            model.size_vram_gb = loaded_ollama[model.name][
                                "size_vram_gb"
                            ]
    ...
    def get_models_for_preset(
        self, preset_id: str
    ) -> dict[str, DiscoveredModel | None]:
        """
        ...
        for model_type in ["fast", "reasoning", "code"]:
            ...
            partial = next(
                (
                    m
                    for m in all_models
                    if target_model in m.name or m.name in target_model
                ),
                None,
            )
            if partial:
            ...
            if preset_id == "light":
                # Prefer small models
                result[model_type] = next(
                    (m for m in all_models if m.speed_tier == "very_fast"),
                    all_models[0] if all_models else None,
                )
            elif preset_id == "balanced":
                # Prefer medium models
                result[model_type] = next(
                    (m for m in all_models if m.speed_tier == "fast"),
                    all_models[0] if all_models else None,
                )
            else:  # heavy
                # Prefer larger models
                result[model_type] = next(
                    (m for m in all_models if m.speed_tier == "slow"),
                    all_models[0] if all_models else None,
                )
        ...
        for preset_id, preset in DEFAULT_PRESETS.items():
            ...
            recommendations[preset_id] = {
                ...
                "models": {
                    ...
                    "reasoning": models["reasoning"].name
                    if models["reasoning"]
                    else None,
...
def get_models_for_preset(preset_id: str) -> dict[str, DiscoveredModel | None]:
    """Get best available models for a preset."""
