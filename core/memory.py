...
class MemoryManager:
    ...
    def __init__(self, persist_path: str = None, max_memories: int = None):
        # Default dimension
        self.d = 384
    ...
    def _compress_old_memories(self):
        ...
        if len(self.documents) <= self.max_memories // 2:  # Only compress if we're significantly over limit
            return
            
        target_size = self.max_memories // 2  # Compress down to half the max
        ...
        if self.documents:
            ...
            # Create new index
            new_index = faiss.IndexFlatL2(self.d)
            if len(vecs) > 0:
                new_index.add(vecs)
            ...
            for i in range(len(self.documents)):
                new_access_order[i] = self.access_order.get(i, time.time())
    ...
    def add_memory(self, text: str, auto_save: bool = False) -> None:
        ...
        self.access_order[doc_index] = time.time()
        
        # Move to end of OrderedDict to mark as most recently used
        if doc_index in self.access_order:
            self.access_order.move_to_end(doc_index)
        ...
        if 0 <= lru_key < len(self.documents):
            ...
            if self.documents:
                ...
                for i in range(len(self.documents)):
                    ...
                    old_index = None
                    # Count how many items were removed before this position
                    removed_before = sum(1 for idx in range(lru_key) if idx in self.access_order)
                    new_access_order[i] = list(self.access_order.values())[i] if i < len(self.access_order) else time.time()
                self.access_order = new_access_order
            else:
                self.index = faiss.IndexFlatL2(self.d)
                self.access_order.clear()
        ...
        for idx, i in enumerate(I[0]):
            if i >= 0 and i < len(self.documents):
                ...
                if i in self.access_order:
                    self.access_order[i] = time.time()
                    self.access_order.move_to_end(i)  # Mark as most recently used
