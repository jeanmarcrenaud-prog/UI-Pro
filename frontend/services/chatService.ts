...
class ChatService {
  ...
  private handleStep(stepId: string, status: string, content?: string, duration?: number, tokenCount?: number): void {
    events.emit('agentStep', { stepId, status: status as 'pending' | 'active' | 'done', content, duration, tokenCount })
  }
