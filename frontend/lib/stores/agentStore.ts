...
interface AgentStore {
  ...
  updateStep: (id: string, status: AgentStepStatus, detail?: string, duration?: number, tokens?: number) => void
...
export const useAgentStore = create<AgentStore>((set, _get) => ({
  ...
  updateStep: (id, status, detail, duration, tokens) =>
    set((state) => {
      ...
      if (stepIdx === -1) {
        return {
          steps: [...state.steps, { id, title: id.replace('step-', '').replace('-', ' '), status, detail, duration, tokens }],
          currentStepId: id,
      ...
      return {
        steps: state.steps.map((s) =>
          s.id === id ? { ...s, status, detail: detail ?? s.detail, duration: duration ?? s.duration, tokens: tokens ?? s.tokens } : s
        ),
        currentStepId: id,
        currentStep: stepIdx
      }
